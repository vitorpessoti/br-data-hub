...
export default function DropzoneComponent() {
  ...
  return (
    <ComponentCard title="Upload de arquivos">
      <div>
        <div
          {...getRootProps()}
          className={`relative flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed p-7 text-center outline-hidden transition-all duration-200 lg:p-10 ${getBorderColor()}`}
        >
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              {files.map((file) => (
                ...
                >
                  ...
                  <div className="p-3">
                    <p
                      className="truncate text-theme-xs font-medium text-gray-800 dark:text-white/90"
                      title={file.name}
                    >
                      {file.name}
                    </p>
